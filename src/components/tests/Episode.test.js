...
test("renders the summary test passed as prop", () => {
    ...
    const summary = screen.queryAllByTestId('summary')
    expect(summary).toHaveLength(1)
});

test("renders default image when image is not defined", () => {
    const myExampleEpisodeData = {...exampleEpisodeData, image: null}
    render(<Episode episode={myExampleEpisodeData} />)
    const imageSelector = screen.queryAllByTestId('image')
